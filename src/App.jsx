      ...
      <div style={{display:"flex",gap:0,background:"rgba(255,255,255,.05)",borderRadius:10,padding:4,width:"100%",maxWidth:400}}>
        {[["join","🔑 Rejoindre"],["create","✨ Créer"]].map(([id,lbl])=>(
          <button key={id} onClick={()=>{setLoginTab(id);setLoginMsg("");}} style={{
            flex:1,padding:"9px",border:"none",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:600,
            background:loginTab===id?"white":"transparent",
            color:loginTab===id?"#1e293b":"#64748b",
            boxShadow:loginTab===id?"0 1px 4px rgba(0,0,0,.2)":"none",
          }}>{lbl}</button>
        ))}
      </div>
      ...
      <div style={{width:"100%",maxWidth:400,background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.1)",borderRadius:14,padding:24,display:"flex",flexDirection:"column",gap:10}}>
        {loginTab==="join" ? <>
          ...
          <input value={inputCode} onChange={e=>setInputCode(e.target.value.toUpperCase())}
            onKeyDown={e=>e.key==="Enter"&&joinService()}
            placeholder="RHUMA01"
            style={{padding:"12px 14px",borderRadius:8,border:"1px solid rgba(255,255,255,.15)",background:"rgba(255,255,255,.07)",color:"white",fontSize:16,fontFamily:"monospace",letterSpacing:2,textAlign:"center",outline:"none"}}
          />
          <button onClick={joinService} disabled={loginBusy} style={{
            padding:"13px",borderRadius:8,border:"none",cursor:"pointer",fontSize:14,fontWeight:700,
            background:"linear-gradient(135deg,#2563eb,#0891b2)",color:"white",marginTop:4,
            opacity:loginBusy?0.7:1,
          }}>{loginBusy?"⏳ Recherche…":"→ Entrer"}</button>
        </> : <>
              ...
              <div style={{overflowX:"auto"}}>
                <table style={{borderCollapse:"collapse",fontSize:10,tableLayout:"fixed"}}>
                  <thead><tr>
                    ...
                    {Array.from({length:days},(_,i)=>{ const d=i+1,dw=getDow(year,month,d),we=isWE(dw);
                      return <th key={d} style={{...PTH,width:21,padding:"2px 0",background:we?"#1a0800":"#0c1625",color:we?"#f97316":"#475569",fontSize:8}}>{d}<br/><span style={{fontSize:7}}>{JOURS_FR[dw].slice(0,2)}</span></th>;
                    })}
                  </tr></thead>
                  <tbody>
                    {g.membres.map((m,mi)=>(
                        ...
                        {Array.from({length:days},(_,i)=>{ const d=i+1,dw=getDow(year,month,d),we=isWE(dw),code=conges[ck(g.id,mi,d)]||"",ci=CODES.find(c=>c.code===code),isAuto=autoMode&&g.id==="paramedical"&&code==="G";
                          return <td key={d} style={{...PTD,width:21,padding:0,background:we?"rgba(40,12,0,.5)":isAuto?"rgba(239,68,68,.05)":"transparent"}}>
                            <input value={code} maxLength={3} onChange={e=>setCode(g.id,mi,d,e.target.value)}
                              style={{width:21,height:20,border:"none",background:"transparent",textAlign:"center",fontSize:9,fontWeight:700,outline:"none",color:ci?ci.color:we?"#2d1a0a":"#334155",fontStyle:isAuto?"italic":"normal",cursor:"text"}}/>
                          </td>;
                        })}
                      </tr>
                    ))}
            ...
            <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:20}}>
              {ordre.map((eq,i)=>(
                <div key={eq} draggable onDragStart={()=>setDragEq(eq)} onDragOver={e=>e.preventDefault()} onDrop={()=>dropEq(eq)}
                  style={{display:"flex",alignItems:"center",gap:10,padding:"12px 20px",borderRadius:10,cursor:"grab",userSelect:"none",
                    background:eq===eqDebut?"rgba(239,68,68,.18)":"rgba(255,255,255,.05)",
                    border:`2px solid ${eq===eqDebut?"#ef4444":"rgba(255,255,255,.12)"}`,
                    boxShadow:dragEq===eq?"0 0 0 3px #ef444444":"none"}}>
        ...
        {tab==="chat"&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",padding:16}}>
            ...
            <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:7}}>
              {["BOUZIANE en congé du 5 au 12","HAMDI en récup le 20","Qui est en garde le 15 ?"].map(s=>(
